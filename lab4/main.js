...
function initBoard() { //создание таблицы
...
function checkAvailableSteps() { //наличие пустых клеток
...
function newGame(){ //запуск новой игры
    let cells = document.querySelectorAll('.cell');
    for (let cell of cells) {
        cell.innerHTML = '';
    }
...
function findWinner() { //поиск победителя
    let cells = document.querySelectorAll('.cell');
    // 0 1 2
    // 3 4 5
    // 6 7 8
...
function clickHandler(event) { //обработчик клика
    if (gameOver == 1) {
...
function showMessage(text, category = 'success') { //вывод сообщения 
