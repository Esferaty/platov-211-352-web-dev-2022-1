...
function compile(str) { // (2+2)*3 -> 22+3*
    let out = [];
    let stack = [];
...
function evaluate() {
    let comp = compile(getDisplay().innerText).split(" ");
    let stack =[];
    console.log(comp);
    if(comp === ''){
    ...
    for(let i=0; i<comp.length; i++) {
        if(!isNaN(comp[i]) && isFinite(comp[i])) {
            stack.push(comp[i]);
        }
        else {
            let a = stack.pop();
...
window.onload = function () {
    ...
    for (let key of keys) key.onclick = clickHandler;
